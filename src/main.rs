...
impl Pendulum {
    ...
    fn update_draw(mut self, settings: Settings, transform: Matrix2d, gl: &mut GlGraphics) -> Pendulum {
        let a1 = self.a1;
        let a2 = self.a2;
        let a1_v = self.a1_v;
        let a2_v = self.a2_v;
        ...
        let line_t = Line::new([self.color.get_red() as f32 / 255.0,
                                   self.color.get_blue() as f32 / 255.0,
                                   self.color.get_green() as f32 / 255.0, settings.pend_transp as f32], settings.pend_width);

        let mag = settings.mag;

        line_t.draw([0.0, 0.0, x1 * mag, y1 * mag], &DrawState::default(), transform, gl);
        line_t.draw([x1 * mag, y1 * mag, x2 * mag, y2 * mag], &DrawState::default(), transform, gl);
...
impl App {
    fn render(&mut self, args: &RenderArgs) {
        ...
        self.gl.draw(args.viewport(), |c, gl| {
            ...
            for ui in 0..self.pends.len() { //((time/0.5) as u8)  {
...
pub fn main() {
    ...
    for i in 0..args.len() {
        match args[i].as_str() {
            "-h" | "--help" => {
                ...
                println!("  -p, --pendulums\t\tNumber of pendulums to simulate. [{}]", amt_pend);
                println!("  -s, --separation\t\tSeparation between pendulums. [{}]", amt_sep);
                println!("  -m1, --mass1\t\t\tMass of pendulum part 1. [{}]", settings.m1);
                println!("  -m2, --mass2\t\t\tMass of pendulum part 2. [{}]", settings.m2);
                println!("  -mag, --magnification\t\tPosition multiplier. [{}]", settings.mag);
                println!("  -g, --gravity\t\t\tGravity/Speed[ish]. [{}]", settings.g);
                println!("  -pt, --transparency\t\tTransparency of each pendulum. [{}]", settings.pend_transp);
                println!("  -pw, --width\t\t\tLine width of pendulums. [{}]\n", settings.pend_width);
    ...
    let mut window: Window = WindowSettings::new(format!("Double Pendulum Simulator! [{} pends, {} deg]", amt_pend, amt_sep), [600, 600])
        .graphics_api(opengl)
        .exit_on_esc(true)
        .build()
        .unwrap();
    ...
    for i in 0..amt_pend {
        pends.push(Pendulum::new(-2.0 + (i as f64 * (amt_sep / amt_pend as f64)), -2.0 + (i as f64 * (amt_sep / amt_pend as f64)),
                                 Hsl::from(i as f32 * (360.0 / amt_pend as f32), 100.0 as f32, 50.0 as f32).to_rgb()));
    }
